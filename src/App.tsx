...
export default function App() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setAuthError(err.message || "Google sign in failed. Check your browser pop-up settings.");
    } finally {
      setAuthLoading(false);
    }
  };
  ...
  const handleUpdateProfile = async (updatedData: Partial<UserProfile>) => {
    ...
    try {
      ...
      console.warn("Supabase auth metadata sync warning:", e);
    }
    setCurrentUser(newProfile);
    
    // Save in local profile cache
    const profiles = JSON.parse(localStorage.getItem('xur_local_profile') || '{}');
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-[#0d1322] rounded-3xl p-6 sm:p-8 max-w-sm w-full shadow-2xl border border-white/10 relative">
            ...
            <button
              ...
              className="w-full bg-white hover:bg-slate-100 text-slate-900 font-bold text-xs py-2.5 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 mb-4 shadow-md active:scale-95"
              id="auth-google-btn"
            >
            ...
            <div className="flex items-center gap-3 my-4">
              ...
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">or email</span>
              <div className="h-px bg-white/10 flex-1" />
            </div>
